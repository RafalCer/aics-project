...
\begin{itemize}
	...
	% We perform several experiments in which we modify visual input to the model.
	...
	% Similar to our experiment on visual abstractions, we also look at what type of augmentation can be made on the language side in this model.
	\pause
...
% also, generally, macro scores are in a lower range than micro scores, demonstrating that the model does worse on non-majority (minority?) classes.
% SD: we can say that there is a discrepancy between the performance on classes +++

% So the model can predict some of the most frequent features (female, attractive), but fails to predict rare features (goatee, receding_hairline). This result indicates that a more complex model is required to operate with augmented visual representations.
...
% SD: we speculate that this enriches the semantic space, introduces additional semantic distinctions between the vocabulary.
